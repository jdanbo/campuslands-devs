...
## Explicación de las decisiones que tomé:
**Evidencia de validación:** En lugar de simplemente retornar los valores en el vacío, decidí implementar `console.table()` y `console.log()` estructurados. Esto permite que, al ejecutar el archivo con Node.js en la terminal (ej: `node solucion_logica_autos.js`), se imprima un reporte limpio, fácil de leer y que demuestra que la lógica funciona correctamente para el club de autos.

**Inmutabilidad:** En el paso 5, utilicé `[...catalogo].sort()` en lugar de `catalogo.sort()`. Esto es fundamental en JavaScript moderno para evitar modificar (mutar) la lista original accidentalmente al momento de ordenarla.

**Presentación de datos:** Para el precio promedio, agregué el objeto `Intl.NumberFormat`. Aunque no era un requisito estricto, darle formato de moneda ($1,234,567.00) al número crudo aporta mucho valor a un sistema diseñado para usuarios de negocios o clientes de lujo.